classdef Col_Linear_UR3 < handle
    properties
        %> Robot model
        model;
...
function self = Col_Linear_UR3(useGripper)
...
%     if nargin < 1
        ...
        pause(0.001);
        name = ['Col_LinearUR_3_',datestr(now,'yyyymmddTHHMMSSFFF')];
%     end
    ...
    L(7) = Link('d',0.185+0.0819,'a',0,'alpha',0,'qlim',deg2rad([-360,360]), 'offset', 0);
...
function PlotAndColourRobot(self)%robot,workspace)
    for linkIndex = 0:self.model.n
        if self.useGripper && linkIndex == self.model.n
            [ faceData, vertexData, plyData{linkIndex+1} ] = plyread(['Col_LinUR3Link',num2str(linkIndex),'Gripper.ply'],'tri'); %#ok<AGROW>
        else
            [ faceData, vertexData, plyData{linkIndex+1} ] = plyread(['Col_LinUR3Link',num2str(linkIndex),'.ply'],'tri'); %#ok<AGROW>
        end
